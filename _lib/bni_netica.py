...
class Node(Node):
	...
	def state(self, name = None, value = None, case = True):
	...
	def experience(self, experience = 1, parentStates = None):
...
class UserProperties(object):
	...
	def add(self, name, value):
		...
		g.SetNodeUserField_bn(self.node.eId, name, value, len(value), 0)

	##OW fix 13/01/21
	##self.eId -> self.node.eId
	def get(self, name):
		# Will return null terminated string if absent; return 'None' in that case
		return str(g.GetNodeUserField_bn(self.node.eId, name, None, 0)) or None
