...
def patchDll(dllFn, lic1, lic2):
	...
	with open(dllFn, 'rb') as inDll:
		with open(dllFn+'.out', 'wb') as outDll:
			...
			lic1Tail = b'BayesFusion activation server'
...
#g.WriteFile.restype = VOID
g.WriteFile.argtypes = [ctypes.POINTER(VOID), c_simplechar_p]
#g.ReadFile.restype = VOID
g.ReadFile.argtypes = [ctypes.POINTER(VOID), c_simplechar_p]
...
g.nodeDefinition_GetStringDefinition.restype = ctypes.POINTER(VOID)
g.nodeDefinition_GetStringDefinition.argtypes = [ctypes.POINTER(VOID), ctypes.c_int]
...
#g.nodeDefinition_SetStringDefinition.restype = VOID
g.nodeDefinition_SetStringDefinition.argtypes = [ctypes.POINTER(VOID), ctypes.c_int, ctypes.POINTER(c_simplechar_p)]
...
g.learn_pc.argtypes = [ctypes.POINTER(VOID), c_simplechar_p, ctypes.c_int, ctypes.c_int, ctypes.c_double]
...
class Net(Net):
	...
	def learnStructure(self, fileName, type = 'PC',
		...
		if type == 'PC':
			matrix = g.learn_pc(self.eNet, fileName, maxAdjacency, maxSearchTime, significance)
			matrixSize = matrix[0]
			...
			for i in range(numNodes):
				...
				for j in range(numNodes):
					pcMatrix[i].append(matrix[i*numNodes + j])
					st += f'{matrix[i*numNodes + j]:2} '
				st += '\n'
...
class Node(Node):
	...
	def finding(self, state = None):
		if state is None:
			if self.hasFinding():
				gNodeValue = self._gNodeVal()
				return self.state(g.nodeValue_GetEvidence(gNodeValue))
			else:
				return None
		else:
	...
	def _equationSd(self):
		...
		return g.valEqEvaluation_GetSampleStdDev(gNodeValue)

	# SM: equationSd is preferred
